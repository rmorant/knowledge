...
Sidebar.include({
  ...
  on_attachment_preview: function(e) {
    ...
    if(attachment_extension) {
      show_preview(attachment_id, attachment_url, attachment_extension,attachment_title);
    } else {
      (new Model('ir.attachment')).call('get_attachment_extension', [attachment_id], {})
        .then(function(extension) {
          show_preview(attachment_id, attachment_url, extension);
        });
    }
  },
  update_preview_buttons: function() {
    var self = this;
    return (new Model('ir.attachment')).call('get_attachment_extension',
        [this.$el.find('.oe-sidebar-attachment-preview')
          .map(function() {
      ...
      {}).then(function(extensions) {
          _(extensions).each(function(extension, id) {
            var $element = jQuery('span.oe-sidebar-attachment-preview[data-id="'+ id + '"]');
...
ListView.include({
  reload_content: function() {
    var deferred = this._super.apply(this, arguments), self = this;
    deferred.then(function() {
      ...
      return (new Model('ir.attachment')).call('get_binary_extension',
              ...
              .then(function(extensions) {
                  _(extensions).each(function(extension, id) {
                    var $element = $elements.filter('[data-id="' + id + '"]');
                    if(can_preview(extension)) {
                      $element.attr('data-extension', extension);
                    } else {
                      $element.remove();
...
FieldBinaryFile.include({
  render_value: function() {
    ...
    if(this.get("effective_readonly") && this.get('value')) {
      var self = this;
      (new Model('ir.attachment')).call('get_binary_extension',
          [
